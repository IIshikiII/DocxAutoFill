...
async def import_nodes(request: Request):
  ...
  words_nodes = [{ "id": f"v{id}", "type": "violet", "data": { "label": f"{name[:-5]} <название>.docx", "category": name } } for id, name in enumerate(file_names)]
  cols_nodes = [{ "id": f"g{id}", "type": "green", "data": { "label": value } } for id, value in enumerate(cols)]
  ...
  for id, word_file in enumerate(words):
    ...
    for id2, elem in enumerate(doc.get_undeclared_template_variables()):
      word_fill_nodes.append({ "id": str(id) + str(id2), "type": "blue", "data": { "label": elem, "category": word_file.filename, } })
